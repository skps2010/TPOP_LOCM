...
int main()
{
    while (1) {
        ...
        std::vector<CreatureCard>
            board[4];  // myLeft myRight opponentLeft
                       // opponentRight
        ...
        int playerHealth, playerMana, playerDeck, playerRune,
            playerDraw;
        int opponentHealth, opponentMana, opponentDeck,
            opponentRune, opponentDraw;

        std::cin >> playerHealth >> playerMana >> playerDeck >>
            playerRune >> playerDraw;
        std::cin >> opponentHealth >> opponentMana >>
            opponentDeck >> opponentRune >> opponentDraw;

        CreatureCard player(
            -1, -1, MY_SIDE, CREATURE, 0, 0, playerHealth,
            "------P", 0, 0, 0, -1, -1);
        ...
        for (int i = 0; i < cardCount; i++) {
            int cardNumber, instanceID, location, cardType, cost,
                attack, defense, myHealthChange,
                opponentHealthChange, cardDraw, lane = LEFT;
                ...
                if (location == IN_HAND) {  // in my hand
                    if (cardType == CREATURE) {
                        CreatureCard summonLeft(
                            ...
                            cardType, cost, attack, defense,
                            abilities, myHealthChange,
                            opponentHealthChange, cardDraw, LEFT,
                            i);
        ...
        // Battle Phase
            ...
            for (const auto &option : cardOptions) {
                ...
                case GREENITEM:
                    if (board[0].size() > 0) {
                        ((ItemCard *) &option)
                            ->use(actions, board[0][0]);
                    } else if (board[1].size() > 0) {
                        ((ItemCard *) &option)
                            ->use(actions, board[1][0]);
                    }
                    break;
                ...
                case REDITEM:
                    ...
                    std::reverse(
                        board[3].begin(), board[3].end());

                    if (board[2].size() > 0 &&
                        board[3].size() > 0) {
                        boardNumber =
                            board[2].back() < board[3].back() ? 2
                                                              : 3;
                    ...
                    if (boardNumber != -1) {
                        ((ItemCard *) &option)
                            ->use(
                                actions,
                                board[boardNumber].back());
                        if (board[boardNumber].back().defense() <=
                            0)
